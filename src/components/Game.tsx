...
export const Game: React.FC<{ board: Board, move: Function, player: number }> = ({ board, move, player }) => {
    ...
    return (
        <div className={`flex ${player === 0 || player === 1 ? 'flex-col-reverse' : 'flex-col'} w-full aspect-square sm:w-[600px] sm:h-[600px] bg-white`}>
            ...
            {board.boardKey?.map((row, i) => {
                return <div className={`flex ${player === 2 ? 'flex-row-reverse' : 'flex-row'} w-full h-[12.5%]`} key={i}>
                    {row?.map((key, j) => {
                        ...
                        return <div className={`flex relative w-[12.5%] h-full ${i % 2 === 0 ? (j % 2 === 0 ? 'bg-sky-500' : '') : (j % 2 === 0 ? '' : 'bg-sky-500')}`} key={j}>
